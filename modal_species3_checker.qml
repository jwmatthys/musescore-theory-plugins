  ...
  */
      ...
      if (v != cantusFirmus && dyads[i].notehead[v] && dyads[i + 1].notehead[v] && dyads[i + 2].notehead[v] && dyads[i + 3].notehead[v] && dyads[i + 4].downbeat) {
        ...
        if (dyads[i].pitch[v] === dyads[i + 3].pitch[v] &&
          ...
          if (dyads[i + 4] && dyads[i + 4].notehead[v] &&
            ...
            dyads[i + 4].voices[v].color = colorInfo;
            var msg = "Double neighbor\nshould resolve\nstepwise, continuing\nin the same\ndirection.";
            markText(dyads[i + 4], msg, colorInfo);
          }
          return true;
        }
      }
      ...
      for (var v = 0; v < 2; v++) {
        if (v != cantusFirmus && dyads[i].pitch[v] && dyads[i + 1].pitch[v] && dyads[i + 2].pitch[v] && dyads[i + 3].pitch[v]) {
          ...
          if (Math.abs(melodicInterval1) > 2 && // 3 consecutive leaps
            ...
            markText(dyads[i + 3], msg, colorConsecutive);
          }
  ...
  function melodicRange(dyads) {
    ...
    for (var v = 0; v < 2; v++) {
      ...
      if (v != cantusFirmus && voiceRange < 10) markText(dyads[0], "Try to cover a\nwider melodic range\nin " + voiceNames[v] + " part.", colorInfo);
    }
  }

// Species2ErrorChecking.js - Error detection for Species 2
// Second species: only passing tones allowed (no neighbor tones, double neighbors, or cambiatas)
...
function checkNonChordTone(noteAna, ana) {
    // In species 2, only passing tones are allowed (no neighbors, double neighbors, or cambiatas)
    if (noteAna.isBass || noteAna.isChordTone) return [];
    
    var isPassingTone = NCT.isPassingTone(noteAna, ana.hData);
    ...
    // Even though it's technically a valid passing tone, we flag it as awkward
    if (noteAna.prevNote && noteAna.nextNote) {
        ...
        // Check for step + aug2 or aug2 + step pattern
        if (sameDirection && 

...
MuseScore {
  menuPath: "Plugins.Proof Reading.Modal Species I Counterpoint Checker"
  ...
  function checkHorizontalIntervals(dyads) {
    for (var i = 0; i < dyads.length - 1; i++) {
      for (var v = 0; v < dyads[i].voices.length; v++) {
        if (dyads[i].tpc[v] && dyads[i + 1].tpc[v]) {
    ...
    for (var i = 0; i < dyads.length - 1; i++) {
      for (var v = 0; v < 2; v++) {
        if (v != cantusFirmus && dyads[i].interval) {
          if (allowedVerticalIntervals.indexOf(dyads[i].interval) < 0 && // dissonance
    ...
    for (var v = 0; v < 2; v++) {
      for (var i = 0; i < dyads.length - 1; i++) {
        if (v != cantusFirmus && dyads[i].notehead[v] && !dyads[i].downbeat) {
          var prev = dyads[i - 1];
          var next = dyads[i + 1];
          console.log("measure", dyads[i].measure, dyads[i].interval, allowedVerticalIntervals.indexOf(dyads[i].interval));

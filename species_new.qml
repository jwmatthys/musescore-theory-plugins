...
MuseScore {
  menuPath: "Plugins.Proof Reading.Counterpoint.Species"
  description: "Check for Errors in Species Counterpoint"
  version: "0.7"
  pluginType: "dialog"
  ...
  var counterpointRestrictions: Object.freeze({
    ...
    Forbid_Dissonant_Downbeats: true, // true in 1st-3rd species
    Forbid_Offbeats: false, // no offbeats, ie first species
    Forbid_All_Dissonance: false,
    Forbid_Voice_Crossing: true,
    Forbid_Accidentals: false,
    Allow_Raised_La_Ti_Minor: false, // exception to Forbid_Accidentals
    Require_Perfect_First_And_Last: true, // generally false for tonal, true for modal
    Forbid_Consecutive_Perfect_Parallels: true, // DONE
    Forbid_Consecutive_Downbeat_Parallels: true, // DONE
    Forbid_Direct_Fifths: true, // refers specifically to P5-d5 or d5-P5 - DONE
    Forbid_Hidden_Parallels: true, // Consecutive parallel P5 or P8 moving in opposite direction - DONE
    Forbid_Leap_To_Perfect_in_Similar_Motion: true, // melody leaps to a perfect interval in similar motion - DONE
    Forbid_Leap_From_Dissonance: true, // a general truism - DONE
    Forbid_Leap_To_Dissonance: true, // exception is Allow_Appoggiatura or Allow_Downward_Appoggiatura - DONE
    Forbid_Melodic_Aug_Or_Dim: true, // DONE
    Forbid_Melodic_Seventh: true, // DONE
    Forbid_Doubled_LT: false, // DONE
    Check_For_Raised_LT_On_V_I: true, // DONE
    Check_For_Ti_Do_Resolution_On_V_I: false, // DONE
    Check_For_Fa_Mi_Resolution_On_V7_I: false, // DONE
    Forbid_Repeated_Note_Over_Barline: false, // strict species forbids this in spec 2 & 3 but not 4 - DONE
    Forbid_Repeated_Offbeats: true, // This is usually true - DONE
    Forbid_Unison_On_Downbeat: true, // Prohibits harmonic unison except beginning and end and offbeats
    ...
    Allow_Nota_Cambiata: false, // DONE
    Allow_Double_Neighbor: false, // DONE
    Allow_Escape_Tone: false, // overrides Forbid_Leap_From_Dissonance DONE
    Check_For_Step_Back_After_Leap: true, // warns if leap of 6th or octave doesn't step back the opposite direction - DONE
  ...
  function cDyad(segment, measure) {
    ...
    this.processFiguredBass = function() {
      ...
      if (this.botNote) {
        if (mode == "Modal") this.consonances = inversion.MODAL;
  ...
  function goSpecies() {
    ...
    for (var index = 0; index < dyads.length; index++) {
      ...
      if (counterpointRestrictions.Require_Perfect_First_And_Last && mode == "Modal") {
      ...
      if (counterpointRestrictions.Forbid_Unison_On_Downbeat) {
      ...
      if (dyads[index].topNote && dyads[index].botNote) {
        if (counterpointRestrictions.Forbid_Offbeats) {
        ...
        if (index > 0 && index < (dyads.length - 1)) // Let's go after those triples (PT, NT, app, ret, sus, apt)
        {
          ...
          if (counterpointRestrictions.Allow_Passing_Tone && !counterpointRestrictions.Forbid_Dissonant_Downbeats) {
            if (dyads[index].nct && dyads[index].botNote && dyads[index].newBot) {
              if (isPassing(dyads[index - 1].topNote, dyads[index].topNote, dyads[index + 1].topNote)) {
          ...
          if (counterpointRestrictions.Allow_Neighbor_Tone && !counterpointRestrictions.Forbid_Dissonant_Downbeats) {
            if (dyads[index].nct && dyads[index].botNote && dyads[index].newBot) {
              if (isNeighbor(dyads[index - 1].topNote, dyads[index].topNote, dyads[index + 1].topNote)) {
        ...
        if (counterpointRestrictions.Allow_Nota_Cambiata && index > 0 && index < dyads.length - 3) {
        ...
        if (counterpointRestrictions.Allow_Double_Neighbor && index > 0 && index < dyads.length - 4) {
          var int1 = new cInterval(dyads[index - 1].topNote, dyads[index].topNote);
          var int2 = new cInterval(dyads[index].topNote, dyads[index + 1].topNote);
          var int3 = new cInterval(dyads[index + 1].topNote, dyads[index + 2].topNote);
        ...
        if (index > 0 && index < (dyads.length - 2) && counterpointRestrictions.Allow_Escape_Tone) {
          var melodicInterval = new cInterval(dyads[index - 1].topNote, dyads[index].topNote);
          var melodicInterval2 = new cInterval(dyads[index].topNote, dyads[index + 1].topNote);
        ...
        if (counterpointRestrictions.Forbid_Dissonant_Downbeats) {
        ...
        if (counterpointRestrictions.Forbid_All_Dissonance) {
        ...
        if (counterpointRestrictions.Forbid_Voice_Crossing) {
        ...
        if (counterpointRestrictions.Forbid_Doubled_LT && mode != "Modal") {
        ...
        if (counterpointRestrictions.Forbid_Accidentals && dyads[index].topNote.accidental) {
          if (!counterpointRestrictions.Allow_Raised_La_Ti_Minor || mode != "Minor") {
          ...
          } else if (counterpointRestrictions.Allow_Raised_La_Ti_Minor && !dyads[index].isRaised6or7) {
            error.annotate(errorMessage.Forbid_Accidentals, colorError);
        ...
        if (counterpointRestrictions.Forbid_Consecutive_Downbeat_Parallels) {
          if (dyads[index].newBot) {
      ...
      if (index > 0) {
        if (counterpointRestrictions.Forbid_Melodic_Aug_Or_Dim) {
        ...
        if (counterpointRestrictions.Forbid_Melodic_Seventh) {
          var melodicInterval = new cInterval(dyads[index].topNote, dyads[index - 1].topNote);
        ...
        if (counterpointRestrictions.Forbid_Consecutive_Perfect_Parallels) {
        ...
        if (counterpointRestrictions.Forbid_Direct_Fifths) {
        ...
        if (counterpointRestrictions.Forbid_Hidden_Parallels) {
        ...
        if (counterpointRestrictions.Forbid_Leap_To_Perfect_in_Similar_Motion) {
          if (dyads[index].interval.isPerfect()) {
            var melodicInterval = new cInterval(dyads[index].topNote, dyads[index - 1].topNote);
        ...
        if (counterpointRestrictions.Forbid_Leap_From_Dissonance) {
        ...
        if (counterpointRestrictions.Forbid_Leap_To_Dissonance) {
          if (dyads[index].isDissonant()) {
            var melodicInterval = new cInterval(dyads[index - 1].topNote, dyads[index].topNote);
            if (melodicInterval.size > 2) {
        ...
        if (counterpointRestrictions.Forbid_Repeated_Offbeats) {
        ...
        if (counterpointRestrictions.Forbid_Repeated_Note_Over_Barline) {
          if (dyads[index].topNote && dyads[index - 1].topNote &&
            dyads[index].botNote && dyads[index].newBot &&
            dyads[index].topNote.pitch == dyads[index - 1].topNote.pitch &&
            dyads[index].topNote.tpc == dyads[index - 1].topNote.tpc &&
        ...
        if (dyads[index - 1].isVchord() && dyads[index].isIchord()) {
          if (counterpointRestrictions.Check_For_Fa_Mi_Resolution_On_V7_I && dyads[index - 1].topNote.sd == 4 && dyads[index].topNote.sd != 3) {
        ...
        if (counterpointRestrictions.Check_For_Raised_LT_On_V_I && index <= dyads.length - 2 &&
        ...
        if (counterpointRestrictions.Check_For_Step_Back_After_Leap && index > 1) {

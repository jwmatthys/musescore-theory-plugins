  ...
  onRun: {
    ...
    for (var index = 0; index < dyads.length; index++) {
      ...
      if (counterpointRestrictions.Perfect_First_And_Last) {
        if ((index == 0 && dyads[index].topNote && !dyads[index].isPerfect()) {
          ...
          } else if ((index == 1 && !dyads[0].topNote && !dyads[index].isPerfect()) { // if it starts with rest
              error.annotate(errorMessage.Perfect_First_And_Last, colorError);
              errorDetails.text += "Measure " + dyads[index].measure + ": Must begin on perfect consonance\n";
            } else if ((index == dyads.length - 1 && !dyads[index].isPerfect()) {
                error.annotate(errorMessage.Perfect_First_And_Last, colorError);

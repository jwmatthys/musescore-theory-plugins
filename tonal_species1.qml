...
MuseScore {
  menuPath: "Plugins.Proof Reading.Counterpoint.Tonal.Tonal Species 1"
  ...
  var counterpointRestrictions: Object.freeze({
    ...
    Unison_On_Downbeat: false, // Prohibits harmonic unison except beginning and end and offbeats
    ...
    Max_Perfect: 40, // percent; warn if too many perfect intervals - DONE
    Max_Leaps: 40, // percent; warn if too many leaps - DONE
  ...
  var errorMessage: Object.freeze({
    ...
    Perfect_First_And_Last: "~P", // generally false for tonal, true for modal
      ...
      if (counterpointRestrictions.Perfect_First_And_Last) {
        if (index == 0 && dyads[index].topNote && !dyads[index].interval.isPerfect()) {
          ...
          if (dyads[index].topNote && dyads[index].botNote) {
            ...
            if (counterpointRestrictions.Nota_Cambiata && index > 0 && index < dyads.length - 3) {

...
MuseScore {
  menuPath: "Plugins.Proof Reading.Modal Species III Counterpoint Checker"
  ...
  */
  ...
  function leapToFromDissonance(dyads) {
    for (var i = 0; i < dyads.length - 1; i++) {
      if (doubleNeighbor(dyads, i)) {
        i += 3;
        continue;
      }
      if (notaCambiata(dyads, i)) {
        i += 3;
        continue;
      }
      for (var v = 0; v < 2; v++) {
  ...
  function doubleNeighbor(dyads, i) {
    // The double neighbor has the following properties:
    // 5 note group (note 0-4)
    // note0 = note3
    // 0-1 = step
    // 1-2 = 3rd in opposite direction
    ...
    for (var v = 0; v < 2; v++) {
      if (v != cantusFirmus && dyads[i].notehead[v] && dyads[i + 1].notehead[v] && dyads[i + 2].notehead[v] && dyads[i + 3].notehead[v] && dyads[i + 4].downbeat) {
        ...
        if (dyads[i].pitch[v] === dyads[i + 3].pitch[v] &&
          isBetween(Math.abs(dyads[i + 1].pitch[v] - dyads[i].pitch[v]), 0, 3) &&
          isBetween(Math.abs(dyads[i + 2].pitch[v] - dyads[i + 1].pitch[v]), 2, 5) &&
          isBetween(Math.abs(dyads[i + 3].pitch[v] - dyads[i + 2].pitch[v]), 0, 3) &&
          sign0to1 === sign2to3 && sign0to1 != sign1to2) {
          console.log("Double neighbor found in " + dyads[i].measure + ".");
          if (dyads[i + 4] && dyads[i + 4].notehead[v] &&
            isBetween(Math.abs(dyads[i + 3].pitch[v] - dyads[i + 2].pitch[v]), 0, 3) &&
            Math.sign(dyads[i + 4].pitch[v] - dyads[i + 3].pitch[v]) === sign0to1) {
            console.log("Double neighbor resolves correctly.");
          } else {
            dyads[i].voices[v].color = colorInfo;
            dyads[i + 1].voices[v].color = colorInfo;
            dyads[i + 2].voices[v].color = colorInfo;
            dyads[i + 3].voices[v].color = colorInfo;
            dyads[i + 4].voices[v].color = colorApproachPerfect; // just for extra attention
            var msg = "Double neighbor\nshould resolve\nstepwise, continuing\nin the same\ndirection.";
            markText(v, dyads[i + 2], msg, colorInfo);
          }
          return true;
        }
      }
  ...
  function notaCambiata(dyads, i) {
    // probably call from leapFromDissonance()
    // there are only three possible essential dissonant cambiatas in third species:
    // Two above the cantus firmus, with vertical intervals 8-7-5-6 (Ex.1) and 6-b5-3-4 (Ex.2);
    // and one below the C.F., with 3-4-6-5 (Ex.3).
    if (i + 4 >= dyads.length) return false;
    if (dyads[i].downbeat && !dyads[i + 1].downbeat && !dyads[i + 2].downbeat && !dyads[i + 3].downbeat &&
      dyads[i + 1].interval && dyads[i + 2].interval && dyads[i + 3].interval) {
      //console.log(dyads[i].interval, dyads[i + 1].interval, dyads[i + 2].interval, dyads[i + 3].interval);
      if (0 === dyads[i].interval &&
        (5 === dyads[i + 1].interval || -2 === dyads[i + 1].interval) &&
        1 === dyads[i + 2].interval &&
        (-4 === dyads[i + 3].interval || 3 === dyads[i + 3].interval)) {
        console.log("8-7-5-6 nota cambiata found in measure " + dyads[i].measure + ".");
        return true;
      } else if ((-4 === dyads[i].interval || 3 === dyads[i].interval) &&
        -5 === dyads[i + 1].interval &&
        (4 === dyads[i + 2].interval || -3 === dyads[i + 2].interval) &&
        -1 === dyads[i + 3].interval) {
        console.log("6-b5-3-4 nota cambiata found in measure " + dyads[i].measure + ".");
        return true;
      } else if ((4 === dyads[i].interval || -3 === dyads[i].interval) &&
        -1 === dyads[i + 1].interval &&
        (-4 === dyads[i + 2].interval || 3 === dyads[i + 2].interval) &&
        1 === dyads[i + 3].interval) {
        console.log("3-4-6-5 nota cambiata found in measure " + dyads[i].measure + ".");
        return true;
      }
    }
    return false;
  ...
  function melodicRange(dyads) {
    ...
    for (var v = 0; v < 2; v++) {
      ...
      if (v != cantusFirmus && voiceRange < 10) markText(v, dyads[0], "Try to cover a\nwider melodic range\nin " + voiceNames[v] + " part.", colorInfo);
    }
  }
